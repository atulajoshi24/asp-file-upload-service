...
public class FileUploadController : ControllerBase
{
    ...
    [HttpPost("secureupload")]
    [RequestSizeLimit(MaxFileBytes)] // Controller-level size guard (works with Kestrel limit)
    public async Task<IActionResult> SecureUpload([FromForm] IFormFile file)
    {
    ...
    public async Task<IActionResult> Upload([FromForm] IFormFile file)
    {
        ...
        var combined = Path.Combine(uploadsRoot, file.FileName);
        Console.WriteLine("combined name  " + combined);
        ...
        var publicUrl = $"/uploads/{file.FileName}";
        Console.WriteLine("publicUrl ..." + publicUrl);
        return Ok(new
        {
            success = true,
            file = new
            {
                id = Path.GetFileNameWithoutExtension(file.FileName), // or DB id
                originalName = Path.GetFileName(file.FileName),
                size = file.Length,
                contentType = file.ContentType,
                url = publicUrl
